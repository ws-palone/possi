        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
				...
				if (emailStudent.equals(emailTeacher)){
					ErrorImport error = new ErrorImport("Ligne " + lineNumber + " : "  + participant.toString(), "L'étudiant et le professeur référent ont le même mail. (doublon)");
					errorsImport.add(error);
				}
				else if (!student.getRole().equals(Role.STUDENT)){
					ErrorImport error = new ErrorImport("Ligne " + lineNumber + " : "  + participant.toString(), "La personne renseigné dans le champ étudiant n'est pas un étudiant reconnu.");
					errorsImport.add(error);
				}
				else if (!followingTeacher.getRole().equals(Role.PROF)){
					ErrorImport error = new ErrorImport("Ligne " + lineNumber + " : "  + participant.toString(), "La personne renseigné dans le champ professeur référent n'est pas un professeur reconnu");
					errorsImport.add(error);
				}
				else if (students.contains(student.getUid())){
					ErrorImport error = new ErrorImport("Ligne " + lineNumber + " : "  + participant.toString(), "L'étudiant est déjà présent dans cet import.");
					errorsImport.add(error);
				}
				else if (participants.contains(participant)) {
					ErrorImport error = new ErrorImport("Ligne " + lineNumber + " : "  + participant.toString(), "La soutenance existe déjà dans cet import.");
					errorsImport.add(error);
				}
				else {
